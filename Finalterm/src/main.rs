...
fn main() -> std::io::Result<()> {
    let matches = App::new("JPEG tutorial")
                          ...
                          .subcommand(SubCommand::with_name("png") //新增png
                               .about("輸出 ppm 格式，檔名固定爲 out.png"))
    ...
    match matches.subcommand_name() {
        ...
        Some("png") => { //Terminal 輸入 jpeg_totorial <imgPath> png 即可執行 png.rs 將解碼後的資料以 png 的格式編碼後輸出
            let image: Image = decoder(reader);
            to_png(image)?;
        }
        Some("mcu") => {
